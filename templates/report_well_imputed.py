...
def well_imputed_by_maf(inWell_imputed, out_prefix, group=''):
    ...
    outWell_imputed_out_1.writelines('\\t'.join(
        [group, '(0,0.001]', '(0.001,0.01]', '(0.01,0.02]', '(0.02,0.05]', '(0.05,0.2]', '(0.2,0.5]', '(0.5,1]',
             'TOTAL']) + '\\n')
    ...
    for line in info_datas:
        ...
        if "SNP" not in line and dataset not in datas:
            ...
            datas[dataset]['extremely_common'] = []
            datas[dataset]['total'] = 0
        if "SNP" in line and "Rsq" in line and 'ALT_Frq' in line:
            idx_exp_freq_a1 = data.index('ALT_Frq')
        else:
            ...
            if maf > 0 and maf <= 0.001:
                ...
                datas[dataset]['extremely_common'].append(maf)
                datas[dataset]['total'] += 1
    ...
    for dataset in datasets:
        ...
        extremely_common = float(len(datas[dataset]['extremely_common']))
        ...
        if extremely_common >= 1000000:
            extremely_common_ = str(
                format(extremely_common / 1000000., '0,.1f')) + 'M ('
        else:
            extremely_common_ = str(format(extremely_common, '0,')) + ' ('
        if tot == 0:
            ...
            outWell_imputed_out_1.writelines('\\t'.join([dataset, \
                                                         ...
                                                         extremely_common_ + str(extremely_common * 100 / tot) + '%)', \
