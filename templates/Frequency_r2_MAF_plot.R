...
# 12.12.2018
# Daniel Schreyer
# Imputation quality score (r2-values) and SNP count plotted against AF bin

#----------------------------------------------------------------
# Generates a scatterplot: x = AF bin,
...
# AF are rounded to 2 decimal places <- bin
Imputed <- Imputed %>% mutate( ALT_Frq = round(as.numeric(ALT_Frq), 2)) %>% group_by(R_Panel, ALT_Frq) %>%
    summarise(Rsq_mean = mean(as.numeric(Rsq)), N = n())

#calculate maximum SNP count to define second y axis
max <- max(Imputed\$N)

# Plot Minor Allele Frequency(ALT_Frq) bin (x), r-squared (y-left) / SNPs count (y-right)
p <- ggplot(Imputed, aes(x = ALT_Frq, color = R_Panel)) +
    geom_point(aes(y = Rsq_mean*max, shape = "Rsq"), size = 1) +
    geom_line(aes(y = Rsq_mean*max)) +
    geom_point(aes(y = N, shape = "Frq"), size = 2) +
    geom_line(aes(y = N))+
    scale_y_continuous(sec.axis = sec_axis(~./max, name = "mean IQS per AF bin",
    breaks = c(seq(0, 1, 0.2))), name = "SNPs count",
    labels = scales::comma) + labs(colour = "") +
    geom_point(aes(x = ALT_Frq, y = Rsq_mean*max, shape = "Rsq", color = R_Panel)
    , inherit.aes = F, size = 1) +
    geom_point(aes(y = N, shape = "Frq"), size = 2) +
    scale_color_npg(name = "Reference Panel") +
    scale_shape_manual(name = "", breaks = c("Rsq", "Frq"),
    labels = c("mean IQS per AF bin","SNPs count" ), values = c(18,4)) +
    theme_bw()
