...
process plot_performance_target{
    ...
    publishDir "${params.outDir}/plots/${target_name}", overwrite: true, mode:'copy'
...
"""
Plot number of imputed SNPs over the mean r2 for all reference panels
dataset, refpanels, infos -- comma separated
"""
process plot_r2_SNPcount {
    tag "plot_r2_SNPcount_${target_name}_${ref_panels.split(',').join('-')}"
    publishDir "${params.outDir}/plots/${target_name}", overwrite: true, mode:'copy'
    label "medium"
    input:
    ...
    script:
        plot_out = "${target_name}_${ref_panels.split(',').join('-')}_r2_SNPcount.pdf"
        impute_info_cutoff = params.impute_info_cutoff
        names = ref_panels
...
process plot_hist_r2_SNPcount {
    tag "plot_hist_r2_SNPcount_${target_name}_${ref_panels.split(',').join('-')}"
    publishDir "${params.outDir}/plots/${target_name}/", overwrite: true, mode:'copy'
    label "medium"
    input:
        tuple val(target_name), val(ref_panels), val(infos)
    output:
        tuple val(target_name), val(ref_panels), file(plot_out)
    script:
        plot_out = "${target_name}_${ref_panels.split(',').join('-')}_r2_SNPcount_hist.pdf"
        impute_info_cutoff = params.impute_info_cutoff
        names = ref_panels
...
"""
process plot_MAF_r2 {
    tag "plot_MAF_r2_${target_name}_${ref_panels.split(',').join('-')}"
    publishDir "${params.outDir}/plots/${target_name}", overwrite: true, mode:'copy'
    label "medium"
    input:
        tuple val(target_name), val(ref_panels), val(infos) 
    output:
        tuple val(target_name), val(ref_panels), file(plot_out) 
    script:
        plot_out = "${target_name}_${ref_panels.split(',').join('-')}_MAF_r2.pdf"
        impute_info_cutoff = params.impute_info_cutoff
