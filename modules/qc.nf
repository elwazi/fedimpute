...
process get_chromosome {
    ...
    input:
        tuple val(dataset), file(dataset_vcf), file(dataset_vcf_idx)
    output:
...
process generate_info_minimac {
    tag "generate_info_minimac_${dataset}"
    label "bigmem"

    input:
        tuple val(dataset), val(refpanel), file(imputed), val(info_in), val(type)
    output:
        tuple val(dataset), val(refpanel), file(imputed), file(info), val(type)
    script:
        base = file(imputed.baseName).baseName
        info = "${base}_${refpanel}.info"
        """
        echo -e "SNP\\tREF(0)\\tALT(1)\\tALT_Frq\\tMAF\\tRsq\\tGenotyped" > ${dataset}_test.txt
        bcftools query -f '%CHROM:%POS:%REF:%FIRST_ALT\\t%REF\\t%FIRST_ALT\\t%INFO/AF\\t%INFO/AF\\t%INFO/R2\\t%INFO/IMPUTED\\n' ${imputed} >> ${dataset}_test.txt
        awk 'BEGIN{ OFS="\\t" } { if (\$7=="."){ \$7="Genotyped" } print }' ${dataset}_test.txt > ${dataset}_replaced.txt
        awk 'BEGIN{ OFS="\\t" } { if (\$7==1){ \$7="Imputed" } print }' ${dataset}_replaced.txt > ${info}
        """
}

process generate_info_sanger {
    tag "generate_info_minimac_${dataset}"
    label "bigmem"
    ...
    script:
        ...
        """
        echo -e "SNP\\tREF(0)\\tALT(1)\\tALT_Frq\\tMAF\\tRsq\\tGenotyped" > ${dataset}_test.txt
        bcftools query -f '%CHROM:%POS:%REF:%FIRST_ALT\\t%REF\\t%FIRST_ALT\\t%INFO/RefPanelAF\\t%INFO/RefPanelAF\\t%INFO/INFO\\t%INFO/TYPED\\n' ${imputed} >> ${dataset}_test.txt
        awk 'BEGIN{ OFS="\\t" } { if (\$7==1){ \$7="Genotyped" } print }' ${dataset}_test.txt > ${dataset}_replaced.txt
        awk 'BEGIN{ OFS="\\t" } { if (\$7=="."){ \$7="Imputed" } print }' ${dataset}_replaced.txt > ${info}
        """
}
...
process check_mismatch {
    tag "check_mismatch_${target}"
    label "medium"
    
    input:
        tuple val(target), file(target_vcf), file(reference_genome)
    output:
        tuple val(target), file(target_vcf), file("${base}_checkRef_warn.log"), file("${base}_checkRef_summary.log")
    script:
        base = file(target_vcf.baseName).baseName
        """
...
process qc_dupl {
    tag "dupl_qc_${dataset}"
    label "bigmem"

    input:
        tuple val(dataset), file(dataset_vcf)
    output:
        tuple val(dataset), file(dataset_qc_vcf)
    script:
        base = file(dataset_vcf.baseName).baseName
        dataset_qc_vcf = "${base}_qc.bcf"
        """
        tabix -f ${dataset_vcf}
...
process filter_min_ac {
    ...
    input:
        tuple val(dataset), file(vcf), val(params)

    output:
        tuple val(dataset), file(vcf_out), file("${vcf_out}.csi"), file(map_file)
